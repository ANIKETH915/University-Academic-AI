...
export default function Sidebar({ activeTab, setActiveTab, isOpen, setIsOpen }) {
  ...
  return (
    <>
      ...
      {/* Sidebar Shell */}
      <aside className={`
        fixed top-0 bottom-0 left-0 z-50 w-[250px] bg-[#0B1020] border-r border-[#1F2937]
      ...
      `}>
        ...
        <div className="p-5 border-b border-[#1F2937] flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="font-heading font-extrabold text-sm tracking-tight text-white">University AI</div>
              <div className="text-[10px] text-slate-400 font-medium">Exam Intelligence</div>
            </div>
          </div>
        ...
        <div className="px-4 py-3 mx-3 my-2 rounded-xl bg-[#080B14] border border-[#1F2937] text-xs flex items-center justify-between">
          <div className="truncate pr-2">
            <div className="text-[9px] uppercase font-bold text-slate-500">Active Workspace</div>
            <div className="font-semibold text-purple-300 truncate">{activeWorkspace?.subject || 'Select a workspace'}</div>
          </div>
          <button
            onClick={() => setIsSelectorOpen(true)}
            className="p-1 rounded-lg bg-[#111827] text-purple-400 hover:text-white transition-colors"
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {item.badge && (
                  <span className="px-1.5 py-0.5 rounded-md bg-purple-500/20 text-purple-300 text-[9px] font-bold uppercase">
                    {item.badge}
                  </span>
                )}
              </button>
        ...
        {/* Footer Tagline */}
        <div className="p-4 border-t border-[#1F2937] text-[11px] text-slate-500 text-center">
