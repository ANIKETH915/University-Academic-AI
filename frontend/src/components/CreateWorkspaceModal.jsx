      ...
      <div className="saas-card w-full max-w-lg p-6 space-y-5 animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="flex items-center justify-between border-b border-slate-200 dark:border-[#1F2937] pb-4">
          <div className="flex items-center space-x-2 text-purple-600 dark:text-purple-400">
            ...
            <div>
              <h3 className="font-heading font-extrabold text-base text-slate-900 dark:text-white">CREATE ACADEMIC WORKSPACE</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">Define a dedicated knowledge space for any course.</p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-slate-50 dark:bg-[#0B1020] border border-slate-200 dark:border-[#1F2937] rounded-xl px-3.5 py-2.5 text-slate-900 dark:text-white focus:border-purple-500 focus:ring-1 focus:ring-purple-500/30"
            />
          </div>
            ...
            <div>
              ...
              >
                {['Semester 1', 'Semester 2', 'Semester 3', 'Semester 4', 'Semester 5', 'Semester 6', 'Semester 7', 'Semester 8'].map(s => (
                  <option key={s} value={s} className="bg-white dark:bg-[#0B1020] text-slate-900 dark:text-white">{s}</option>
                ))}
              </select>
            </div>
          ...
          {/* Buttons */}
          <div className="pt-3 border-t border-slate-200 dark:border-[#1F2937] flex items-center justify-end space-x-3">
            <button
              type="button"
              onClick={() => setIsCreateOpen(false)}
              className="px-4 py-2 rounded-xl bg-slate-100 dark:bg-[#0B1020] border border-slate-200 dark:border-[#1F2937] text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white text-xs font-semibold"
              disabled={isCreating}
            >
              Cancel
            </button>
