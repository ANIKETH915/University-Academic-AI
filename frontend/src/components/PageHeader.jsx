    ...
    <div className="saas-card p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <div className="flex items-center space-x-2 text-xs font-semibold text-slate-400 mb-1">
          <span className="text-slate-400">University Academic AI</span>
          <ChevronRight className="w-3 h-3 text-slate-600" />
          <span className="text-purple-400 font-semibold">{breadcrumb}</span>
          {badge && (
            <span className="ml-2 text-[10px] font-extrabold px-2 py-0.5 rounded bg-purple-500/20 text-purple-300 border border-purple-500/30">
              {badge}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center space-x-3 mt-1">
          ...
          <div>
            <h1 className="font-heading font-extrabold text-2xl text-white tracking-tight leading-tight">
              {title}
            </h1>
            {subtitle && (
