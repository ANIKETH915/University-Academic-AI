"""Trace the 2-page 2023 May NLP PDF."""
...
PDF = (
    ...
    r"\be_computer-engineering_semester-7_2023_may_"
...
def eval_repr(name, text, page_no):
    ...
    acc, rej = extract_questions_from_page_text(
        prepared, page_no, os.path.basename(PDF), "ws", year=2023
    )
    marks = detect_source_question_markers(prepared)
    print(
        f"  {name:16s} chars={len(text):5d} markers={marks} acc={[q['question_id'] for q in acc]} "
        f"struct={question_structure_score([q['question_id'] for q in acc]):.3f}"
    )
    for r in rej[:10]:
        print(f"      rejected {r.get('question_id')} <- {r.get('reason')} | {(r.get('raw_text') or '')[:140]}")
    return prepared
...
def main():
    ...
    for i, page in enumerate(doc):
        ...
        print(f"\n=== PAGE {page_no} native_valid={valid} {reason} ===")
        print("ocr:\n", ocr[:1800])
        print("--- classified ---")
        for ln in layout_lines:
            kind, parent, sub, rest = _classify(ln["text"])
            print(f"  x0={ln['x0']:4d} {kind:12s} p={str(parent):4s} s={str(sub):4s} {ln['text'][:140]}")
        print("--- reconstructed ---")
        ...
        prep = eval_repr("layout", layout, page_no)
        pages_payload.append({
            "page": page_no,
            "raw_native_text": native,
            "raw_ocr_text": ocr,
            "reconstructed_text": prep or prepare_page_text_for_extraction(layout or filter_noise_lines(ocr) or filtered),
            "ocr_used": True,
        })
    doc.close()
    ...
    print("texts:")
    for q in hybrid["accepted_questions"]:
        print(f"  {q['question_id']}: {q['exact_text'][:180]}")
    print("quality", hybrid["quality"])
    print("rejected", [(r.get("question_id"), r.get("reason"), (r.get("raw_text") or "")[:80]) for r in (hybrid.get("rejected_candidates") or [])])
    print("cross_page", hybrid.get("cross_page_merges"))
